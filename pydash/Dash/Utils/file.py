...
def Upload(dash_context, user, file_root, file_bytes, filename, nested=False, parent_folders=[]):
    if filename.count(".") != 1:
    ...
    file_data = add_default_keys(file_data, user, file_root, parent_folders)
...
def add_default_keys(file_data, user, file_root, parent_folders=[]):
    ...
    file_data["parent_folders"] = parse_parent_folders(file_root, file_id, parent_folders)
